...
def parse(
    ...
    **kwargs
    ...
    """
    parser, handler = __parser_preprocessor(
        xml_input,
        encoding,
        expat,
        process_namespaces,
    ...
    if isasyncgen(xml_input):
        return __async_parse(parser, handler, xml_input)
    return __parse(parser, handler, xml_input)


def __parser_preprocessor(
    xml_input,
    encoding,
    expat,
    process_namespaces,
...
def __parse(parser, handler, xml_input):
...
async def __async_parse(parser, handler, xml_input):
    if isasyncgen(xml_input):
        async for chunk in xml_input:
            parser.Parse(chunk, False)
        parser.Parse(b"", True)
    return handler.item
